...
class _TenderPostScreenState extends State<TenderPostScreen>
  ...
  Future<void> _submitTender() async {
    ...
    setState(() {
      _deadlineError = _deadline == null ? 'Please select a deadline' : null;
      _locationError = _selectedLocationId == null
          ? 'Please select a country'
          : null;
    });

    if (!isFormValid || _deadlineError != null || _locationError != null) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Povećan prag širine za pravi desktop raspored (1100px je idealno za dvo-kolonski layout)
    final bool isDesktop = MediaQuery.sizeOf(context).width >= 1100;
    final int descriptionLines = isDesktop ? 8 : 4;

    // Definišemo sekcije u varijable kako bismo ih lakše presložili u zavisnosti od ekrana
    final detailsCard = AppCard(
    ...
    final deadlineAndDescCard = AppCard(
    ...
    return Scaffold(
      ...
      body: FadeInWrapper(
        child: Center(
          // Centriranje forme na velikim desktop ekranima
          child: ConstrainedBox(
            constraints: const BoxConstraints(
              maxWidth: 1200,
            ), // Ograničenje maksimalne širine desktop forme
  ...
  PreferredSizeWidget _buildAppBar() => AppBar(
    ...
    elevation: 0,
    centerTitle: false,
    // ISPRAVLJENO: Uklonjen mobilni systemOverlayStyle teret
